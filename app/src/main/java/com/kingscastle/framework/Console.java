...
public class Console {
	...
	public static void cmd(  String input ){
		...
		if( input.startsWith("test") ) {
			Game.testingVersion = !Game.testingVersion;
			...
			String[] inputs = input.split(" ");
			//.getLevel().goToRound(Integer.parseInt(inputs[1]));
		}
		...
		//		}
			...
			View.OnClickListener ocl = new View.OnClickListener() {
				...
				public void onClick( View v) {
					...
					Anim a = null;
					try {
						Class<? extends Anim> aClass = (Class<? extends Anim>) Class.forName("com.kingscastle.effects.animations."+ ((Button)v).getText() );
