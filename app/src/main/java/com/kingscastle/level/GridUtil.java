...
import com.kingscastle.gameUtils.VectorUtil;
...
public class GridUtil
{
	 ...
	 */
    @NotNull
	public vector getClosestAdjWalkableTile( @NotNull vector v )
	{
    ...
    /**
     * If there is a walkable loc adjacent to the area it is returned.
    ...
    public vector getAWalkableLocNear(  RectF area ){
        ...
        loc = new vector(area.centerX(), area.centerY());

        float dist = Rpg.sixTeenDp;
        for (int i = 0; i < 50; i++) {

            loc.randomize(dist);
            if( isWalkable(loc, grid))
                return loc;
        }
        return loc;
    }
